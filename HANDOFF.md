...
- **M31（本地，未提交）：联机模式（方案①：房间号 + HTTP 状态中转，异地一房一轮）**
  ...
  - **M31.2 全量枚举回归（2026-09-18）**：从运行时 CATALOG 枚举出 **67 个藏点**（含 4 个书页间、
    6 个 under 薄位、3 个 soil 位、电视柜/衣柜等全部大写槽位），按 8 点/房间分 9 批双端 E2E，
    每点走完整七环链（选中→放置→生成链接→链接生效→找家搜查命中→拾取→房主收到反馈），
    **67/67 全通过，每批房主轮询反馈正常**；物品 8 种（纸条/扑克/硬币/橡皮/钥匙/遥控/戒指/球）
    轮转覆盖（纸条15/硬币9/扑克9/戒指8/球8/橡皮6/遥控6/钥匙5）。canHide 自动适配得到验证：
    薄位（地毯 0.018/床边毯）自动降级为纸条/扑克，书页间强制 paper 类。
    链接自动加入已单独验证（?room= 打开即见确认屏）。回归时发现并修复：
    start_seek 应答缺 state 字段（见上）与 SLOT_RE 大写槽位（见下）。
